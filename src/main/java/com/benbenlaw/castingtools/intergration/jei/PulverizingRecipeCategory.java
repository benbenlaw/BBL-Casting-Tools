...
public class PulverizingRecipeCategory implements IRecipeCategory<PulverizingRecipe> {
    ...
    public static final IRecipeType<PulverizingRecipe> RECIPE_TYPE = IRecipeType.create(CastingTools.identifier("pulverizing"), PulverizingRecipe.class);

    private final int width = 101;
    private final int height = 20;
    private final IDrawable icon;

    public PulverizingRecipeCategory(IGuiHelper guiHelper) {
        icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(Items.MACE));
    }

    @Override
    public @NotNull IRecipeType<PulverizingRecipe> getRecipeType() {
        return RECIPE_TYPE;
    }

    @Override
    public @NotNull Component getTitle() {
        return Component.translatable("jei.castingtools.pulverizing");
    }
    ...
    @Override
    public void createRecipeExtras(IRecipeExtrasBuilder builder, PulverizingRecipe recipe, IFocusGroup focuses) {
    ...
    public void draw(PulverizingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, 0, 0, 0, 0, width, height, width, height);
    }

    @Override
    public void getTooltip(ITooltipBuilder tooltip, PulverizingRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        if (MouseUtil.isMouseAboveArea((int) mouseX, (int) mouseY, 21, 2, 0, 0, 28, 18)) {
            tooltip.add(Component.translatable("tooltip.castingtools.pulverizing", recipe.block().getName()));
        }
    }
}
