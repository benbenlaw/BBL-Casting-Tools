...
# 1.1.0
- Fixed JEI translation
- Added Efficiency modifier, max level 10, each level increases by 0.5f
- Added Unbreaking, max level 10, each level increases chance to not consume durability by 10%, level 10 is unbreaking
- Added Repairing, max level 8, each level repairs faster level one is 10 seconds per repair decreasing by 1 second per level
- Added Torch Placer, places torch on right click
- Added Looting, max level 10, functions the same as vanilla looting enchantment
- Changed tags for modifier to more casting ones to improve coverage of items
- Improved modifier matching system, fluid and item modifier are now checked first
- Added creative tab
- Modifier now shows in creative and jei
- Clearing recipe added craft to clear modifier inventory
- Standing on top of the modifier will now drain the player experience and add it to the modifier internal tank
- JEI now only shows tools as outputs 
